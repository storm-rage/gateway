 ...
 * @LastEditTime: 2025-06-16 14:54:49
 * @Description:
 */
...
export default function SettingStation() {
  ...
  useEffect(() => {
    if (isFirst.current) {
      ...
      setFormItemConfig((prevState) => ({
        ...prevState,
        systemId: { options: getAllBelongSystem || [] },
        deviceType: { options: dvsTypes || [] },
      }))
      const formInst = formRef.current?.getInst()
      const sysList = getStorage(StorageDeviceSystem)?.map((i) => {
        return { label: i.name, value: i.id }
      })
      setBelongList(sysList)
      formInst?.submit()
    }
  }, [])
  ...
  useEffect(() => {
    if (isEditState.current) {
      setColumn(ST_STATION_SYS_COLUMNS(setDataSource, belongList, options, deviceTypeRef.current))
    }
  }, [dataSourceList])
  ...
  const onSchValueChgRef = async (changedValue) => {
    ...
    if (Object.keys(changedValue).includes("deviceType")) {
      deviceTypeRef.current = changedValue?.deviceType
    }
  }
  const formSelectChange = useRef(async (changeVal, setFormConfigs) => {
    if (changeVal?.deviceType) {
  ...
  const btnClkRef = async (type: "ok" | "close", data?) => {
    // 执行
    if (type === "ok") {
      const res = await addPointMethods(data)
      if (!res) return
  ...
  const delBtnClkRef = async (type: "ok" | "close" | "delete_ok", info) => {
    // 执行
    if (type === "delete_ok") {
      const res = await delPointsMethods(selectedRows)
      if (!res) return
      setDelModal(false)
      setSelectedRowKeys([])
      setSelectedRows([])
      ...
      <CustomModal<IOperateProps, IPerateRef>
        ...
        componentProps={{
          ...
          FORM_ITEMS: formItemFuc({
            ...
            currentDvsType: modalFormDvsType,
          }),
        }}
      />
    </div>

 ...
 * @LastEditTime: 2025-06-04 16:41:40
 * @Description:设备模型-线路
 */
import "./index.less"
...
import { IPeriodData, IPeriodDataParam, TModelFrAndTbInfo, TUserTbActInfo } from "./types/index"
...
export default function ModelStation() {
  ...
  // 设置选中的一条数据
  const [selectRowInfo, setSelectRowInfo] = useState<IPeriodData | null>()
  ...
  // 执行查询的钩子
  const { dataSource, loading, pagination, onSearch } = usePageSearch<IPeriodDataParam, IPeriodData>(
  ...
  async function onFormAction(type: TFormType) {
    if (type === "edit" && dataSource?.length) {
      ...
      isEditState.current ? setColumn(PERIOD_EDIT_COLUMNS(setDataSource)) : ""
      !isEditState.current
  ...
  const onTbAction = async (record: IPeriodData, { key, label }: TUserTbActInfo) => {
    setIsModalOpen(key)
  ...
  useEffect(() => {
    if (isEditState.current) {
      setColumn(PERIOD_EDIT_COLUMNS(setDataSource))
    }
  }, [dataSourceList])
  useEffect(() => {
  ...
  return (
    <div className="page-wrap setting-user">
      <CustomForm
        ...
        itemOptions={PERIOD_MANAGE_FORM_ITEMS}
        onSearch={onSearch}

    ...
    getDeviceCodeList().then(res => {
      if (res) {
        ...
        res.forEach((item: any) => {
          if (item.deviceCode && !uniqueCodes.has(item.deviceCode)) {
            uniqueCodes.set(item.deviceCode, { label: String(item.deviceCode), value: item.deviceCode })
          }
        })
        setDeviceCodeList(Array.from(uniqueCodes.values()))
      }
    })
  }, [])

...
export async function getSettingUserSchData(pageInfo?: IPageInfo, formData?: any) {
  ...
  const res = await doBaseServer<IUserListParam>("deviceModelSelectByPage", params)
...
export const addUserMethods = async (data: any, type: TModalType) => {
  const api = type == "add" ? "deviceModelInsert" : "deviceModelUpdate"
...
export const importFile = async (formData) => {
  const res = await doBaseServer("deviceModelImportData", formData)
  return validOperate(res)
}
export const exportFile = async (formData) => {
  doBaseServer<any, AxiosResponse>("deviceModelExportData", formData).then((data) => {
...
export const delUserMethods = async (data: any) => {
  const res = await doBaseServer<any>("deviceModelBatchDelete", data)
  return validOperate(res)
}
